...
contract BowPool is HRC20, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < coins.length; i++) {
            ...
            if (in_amount > 0) {
                ...
                if (i == FEE_INDEX) {
                    in_amount = IHRC20(in_coin).balanceOf(address(this));
                }
                ...
                if (i == FEE_INDEX) {
                    ...
                    in_amount = IHRC20(in_coin).balanceOf(address(this)).sub(
                        in_amount
                    );
                }
            }
    ...
    function exchange(
        ...
        if (i == FEE_INDEX) {
            dx_w_fee = IHRC20(input_coin).balanceOf(address(this));
        }
        ...
        if (i == FEE_INDEX) {
            ...
            dx_w_fee = IHRC20(input_coin).balanceOf(address(this)).sub(
                dx_w_fee
            );
        }
    ...
    function admin_balances(uint256 i) external view returns (uint256 balance) {
        balance = IHRC20(coins[i]).balanceOf(address(this)).sub(balances[i]);
    }

    function withdraw_admin_fees() external onlyOwner {
        for (uint256 i = 0; i < coins.length; i++) {
            address c = coins[i];
            uint256 value = IHRC20(c).balanceOf(address(this)).sub(balances[i]);
            if (value > 0) {
    ...
    function donate_admin_fees() external onlyOwner {
        for (uint256 i = 0; i < coins.length; i++) {
            balances[i] = IHRC20(coins[i]).balanceOf(address(this));
        }
    }
