...
contract HRC20 is Context, IHRC20 {
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "HRC20: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _transfer(
        ...
        _balances[sender] = _balances[sender].sub(
            amount,
            "HRC20: transfer amount exceeds balance"
        );
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _balances[account] = _balances[account].sub(
            amount,
            "HRC20: burn amount exceeds balance"
        );
...
// File: contracts/BowTokenWallet.sol
...
// File: contracts/interfaces/IBowPool.sol

pragma solidity ^0.6.0;


interface IBowPool is IHRC20 {
...
// File: contracts/interfaces/IBowProxy.sol
...
// File: contracts/interfaces/IBowToken.sol
...
contract BowProxy is IBowProxy, HRC20, Ownable, ReentrancyGuard {
    ...
    function createWallet() internal {
        ...
        walletShare = new BowTokenWallet(
            "Bow Token Wallet for LP farming reward",
            "BTWL",
            address(this)
        );
        walletSwap = new BowTokenWallet(
            "Bow Token Wallet for LP swap reward",
            "BTWS",
            address(this)
        );
        walletLPStaking = new BowTokenWallet(
    ...
    function getWallets()
        ...
        override
        returns (
            BowTokenWallet wsAddress,
            BowTokenWallet weAddress,
            BowTokenWallet wstakingAddress
        )
    {
        ...
        returns (uint256)
    {
        ...
        uint256 lpSupply =
            IHRC20(pool.poolAddress).balanceOf(address(walletLPStaking));
        if (lpSupply != 0) {
            uint256 releaseAmt =
    ...
    function deposit(uint256 _pid, uint256 _amount) external noOpenMigration {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                uint256 tokenBal =
                    IHRC20(tokenAddress).balanceOf(address(walletShare));
                if (tokenBal >= pending) {
